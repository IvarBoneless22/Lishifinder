(()=>{
  ...
  function comparisonRecords(){
    const internal=filteredRecords();const externalOnly=externalFilteredRecords().filter(external=>!catalog.some(record=>sameVehicle(record,external))).map(record=>({...record,_externalOnly:true}));return[...internal,...externalOnly].sort((left,right)=>(left.brand+" "+left.model+" "+left.years).localeCompare(right.brand+" "+right.model+" "+right.years,"uk"));
  }
  ...
  function makeComparisonPanel(record){
    ...
    if(record._externalOnly){summary.className="comparisonSummary pending";summary.textContent="Зовнішній запис відсутній у вашому каталозі LishiFinder.";}else if(!matches.length){summary.className="comparisonSummary pending";summary.textContent="Зовнішні записи не імпортовані або відповідність не знайдена.";}else if(ownCode&&codes.every(code=>code===normalize(ownCode))){summary.className="comparisonSummary confirmed";summary.textContent="Збіг: усі імпортовані джерела вказують той самий Lishi.";}else{summary.className="comparisonSummary conflict";summary.textContent="Є розбіжність між вашим каталогом та зовнішніми джерелами.";}panel.append(summary);
