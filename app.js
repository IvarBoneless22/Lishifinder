(()=>{
  ...
  const elements={query:$("q"),brand:$("brand"),brandPicker:$("brandPicker"),brandButton:$("brandButton"),brandButtonIcon:$("brandButtonIcon"),brandButtonText:$("brandButtonText"),brandMenu:$("brandMenu"),market:$("market"),ignition:$("ignition"),year:$("year"),tool:$("tool"),results:$("results"),stats:$("stats"),chips:$("brandChips"),modeHelp:$("modeHelp"),showAll:$("showAll"),clear:$("clr"),reset:$("resetApp"),updateStatus:$("updateStatus"),photoDialog:$("photoDialog"),photoTitle:$("photoTitle"),photoClose:$("photoClose"),photoLoading:$("photoLoading"),photoImage:$("photoImage"),photoMessage:$("photoMessage"),photoCredit:$("photoCredit"),photoCreditText:$("photoCreditText"),photoSource:$("photoSource"),manageSources:$("manageSources"),sourceCount:$("sourceCount"),sourcesDialog:$("sourcesDialog"),sourcesClose:$("sourcesClose"),sourceFile:$("sourceFile"),clearSources:$("clearSources"),exportData:$("exportData"),importStatus:$("importStatus"),builtinCatalogSummary:$("builtinCatalogSummary"),importedSourceSummary:$("importedSourceSummary"),importedSourceState:$("importedSourceState")};
  const STORAGE_KEYS={external:"lishi-finder-external-v1",notes:"lishi-finder-notes-v1"};
  const state={mode:"auto",showAll:false,photoRequest:0,externalRecords:[],notes:{}};
  ...
  const sourceRank=record=>record.source==="Скріншот користувача"?0:1;
  const sortRecords=(left,right)=>sourceRank(left)-sourceRank(right)||(left.brand+" "+left.model+" "+left.years).localeCompare(right.brand+" "+right.model+" "+right.years,"uk");
  const sourceLinks=[{name:"Original Lishi",url:"https://www.originallishi.com/lishi-tools-full-list/"},{name:"Genuine Lishi",url:"https://www.genuinelishi.com/"},{name:"AKS Reference Guide",url:"https://fliphtml5.com/aswuk/juqd/American_Key_Supply_-_Lishi_Reference_Guide_2024/"}];
  function readStoredJson(key,fallback){try{const value=localStorage.getItem(key);return value?JSON.parse(value):fallback;}catch(error){return fallback;}}
  function storeJson(key,value){try{localStorage.setItem(key,JSON.stringify(value));return true;}catch(error){return false;}}
  function recordKey(record){return [record.brand,record.model,record.years,(record.markets||[]).join(","),record.ignition,record.lishi].map(normalize).join("|");}
  function baseModel(value){return normalize(value).replace(/\([^)]*\)/g," ").replace(/\b(key|start|prox|proximity|smart|slot|remote)\b/g," ").replace(/[^\p{L}\p{N}]+/gu," ").replace(/\s+/g," ").trim();}
  function yearsOverlap(left,right){const leftFrom=Number(left.yearFrom)||0,leftTo=Number(left.yearTo)||9999,rightFrom=Number(right.yearFrom)||0,rightTo=Number(right.yearTo)||9999;if(!leftFrom||!rightFrom)return true;return leftFrom<=rightTo&&rightFrom<=leftTo;}
  function sameVehicle(left,right){if(normalize(left.brand)!==normalize(right.brand))return false;const leftModel=baseModel(left.model),rightModel=baseModel(right.model);if(!leftModel||!rightModel)return false;return (leftModel===rightModel||leftModel.includes(rightModel)||rightModel.includes(leftModel))&&yearsOverlap(left,right);}
  function safeSourceUrl(value){try{const url=new URL(String(value||""));return ["http:","https:"].includes(url.protocol)?url.href:"";}catch(error){return "";}}
  function yearRange(raw){
    const text=String(raw.years||raw.year||raw.year_range||raw["роки"]||"").trim();const found=text.match(/(19|20)\d{2}/g)||[];const from=Number(raw.yearfrom||raw.yearFrom||raw.year_from||found[0])||null;const to=Number(raw.yearto||raw.yearTo||raw.year_to||found.at(-1))||from;
    return{years:text||(from?(from===to?String(from):from+"-"+to):""),yearFrom:from,yearTo:to};
  }
  function firstField(raw,names){for(const name of names){if(raw[name]!==undefined&&String(raw[name]).trim())return raw[name];}return "";}
  function normalizeImportedRecord(raw,defaultSource,index){
    const normalizedRaw={};Object.entries(raw||{}).forEach(([key,value])=>{normalizedRaw[normalize(key).replace(/\s+/g,"_")]=value;});
    const brand=String(firstField(normalizedRaw,["brand","make","марка","manufacturer"])).trim();const model=String(firstField(normalizedRaw,["model","vehicle","модель","автомобіль"])).trim();const lishi=String(firstField(normalizedRaw,["lishi","tool","reference","keyway","код","інструмент"])).trim().toUpperCase();if(!brand||!model||!lishi)return null;
    const range=yearRange(normalizedRaw);const marketValue=firstField(normalizedRaw,["market","markets","ринок"]);const markets=Array.isArray(marketValue)?marketValue.map(String):String(marketValue||"").split(/[;,|]/).map(value=>value.trim()).filter(Boolean);
    return{brand,model,years:range.years,yearFrom:range.yearFrom,yearTo:range.yearTo,markets,ignition:String(firstField(normalizedRaw,["ignition","key_type","тип_ключа"])||"unknown"),lishi,status:"verified",source:String(firstField(normalizedRaw,["source","database","джерело"])||defaultSource||"Імпорт").trim(),sourceUrl:safeSourceUrl(firstField(normalizedRaw,["sourceurl","source_url","url","посилання"])),note:String(firstField(normalizedRaw,["note","notes","примітка"])||"").trim(),externalId:"external-"+Date.now()+"-"+index};
  }
  function parseCsv(text){
    const rows=[];let row=[],cell="",quoted=false;for(let index=0;index<text.length;index+=1){const char=text[index],next=text[index+1];if(char==='"'&&quoted&&next==='"'){cell+='"';index+=1;}else if(char==='"'){quoted=!quoted;}else if(char===","&&!quoted){row.push(cell);cell="";}else if((char==="\n"||char==="\r")&&!quoted){if(char==="\r"&&next==="\n")index+=1;row.push(cell);if(row.some(value=>String(value).trim()))rows.push(row);row=[];cell="";}else cell+=char;}row.push(cell);if(row.some(value=>String(value).trim()))rows.push(row);if(rows.length<2)return[];const headers=rows[0].map(value=>String(value).trim());return rows.slice(1).map(values=>Object.fromEntries(headers.map((header,index)=>[header,values[index]||""])));
  }
  function ukCount(count,one,few,many){const lastTwo=count%100,last=count%10;return count+" "+(lastTwo>=11&&lastTwo<=14?many:(last===1?one:(last>=2&&last<=4?few:many)));}
  function updateSourceSummary(){
    const count=state.externalRecords.length;const sources=uniqueSorted(state.externalRecords.map(record=>record.source));elements.sourceCount.textContent=ukCount(count,"зовнішній","зовнішні","зовнішніх");elements.importedSourceState.textContent=String(count);elements.importedSourceSummary.textContent=count?sources.join(", ")+" — "+ukCount(count,"запис","записи","записів"):"Ще не імпортовано";
  }
  async function importSourceFile(file){
    elements.importStatus.classList.remove("error");if(!file)return;if(file.size>2*1024*1024){elements.importStatus.textContent="Файл завеликий. Максимальний розмір — 2 МБ.";elements.importStatus.classList.add("error");return;}
    try{const text=await file.text();const extension=file.name.split(".").pop().toLowerCase();let rawRecords,backupNotes=null;if(extension==="json"){const parsed=JSON.parse(text);rawRecords=Array.isArray(parsed)?parsed:(Array.isArray(parsed.records)?parsed.records:(Array.isArray(parsed.externalRecords)?parsed.externalRecords:[]));if(parsed&&parsed.notes&&typeof parsed.notes==="object"&&!Array.isArray(parsed.notes))backupNotes=parsed.notes;}else rawRecords=parseCsv(text);const sourceName=file.name.replace(/\.[^.]+$/," ").trim()||"Імпорт";const records=rawRecords.map((raw,index)=>normalizeImportedRecord(raw,sourceName,index)).filter(Boolean);if(!records.length)throw new Error("format");state.externalRecords=records;if(backupNotes){state.notes=backupNotes;storeJson(STORAGE_KEYS.notes,state.notes);}if(!storeJson(STORAGE_KEYS.external,records))throw new Error("storage");updateSourceSummary();elements.importStatus.textContent="Імпортовано "+ukCount(records.length,"запис","записи","записів")+(backupNotes?" і відновлено примітки":"")+". Пошук у режимі «Порівняти» оновлено.";elements.sourceFile.value="";state.showAll=true;render();}catch(error){elements.importStatus.textContent="Не вдалося імпортувати файл. Перевірте формат і обов’язкові поля brand, model та lishi.";elements.importStatus.classList.add("error");}
  }
  function downloadJson(){
    const payload={exportedAt:new Date().toISOString(),externalRecords:state.externalRecords,notes:state.notes};const url=URL.createObjectURL(new Blob([JSON.stringify(payload,null,2)],{type:"application/json"}));const link=document.createElement("a");link.href=url;link.download="lishi-finder-my-data.json";link.click();window.setTimeout(()=>URL.revokeObjectURL(url),1000);
  }
  ...
  function openSourcesDialog(){elements.importStatus.textContent="";elements.importStatus.classList.remove("error");if(typeof elements.sourcesDialog.showModal==="function")elements.sourcesDialog.showModal();else elements.sourcesDialog.setAttribute("open","");}
  function closeSourcesDialog(){if(typeof elements.sourcesDialog.close==="function"&&elements.sourcesDialog.open)elements.sourcesDialog.close();else elements.sourcesDialog.removeAttribute("open");}
  ...
  function matchesQuery(record,query){
    ...
    const searchable=state.mode==="lishi"?[record.lishi]:[record.brand,record.model,record.years,...(record.markets||[]),ignitionNames[record.ignition],state.mode==="compare"?record.lishi:""];
  ...
  function filteredRecords(){
    ...
    return catalog.filter(record=>(!elements.brand.value||record.brand===elements.brand.value)&&(!elements.market.value||(record.markets||[]).includes(elements.market.value))&&(!elements.ignition.value||record.ignition===elements.ignition.value)&&(!elements.tool.value||record.lishi===elements.tool.value)&&matchesYear(record,elements.year.value)&&matchesQuery(record,query)).sort(sortRecords);
  }
  function externalFilteredRecords(){
    const query=normalize(elements.query.value);return state.externalRecords.filter(record=>(!elements.brand.value||normalize(record.brand)===normalize(elements.brand.value))&&(!elements.market.value||!(record.markets||[]).length||(record.markets||[]).some(value=>normalize(value)===normalize(elements.market.value)))&&(!elements.tool.value||record.lishi===elements.tool.value)&&matchesYear(record,elements.year.value)&&matchesQuery(record,query));
  }
  function comparisonRecords(){
    const internal=filteredRecords();const externalOnly=externalFilteredRecords().filter(external=>!catalog.some(record=>sameVehicle(record,external))).map(record=>({...record,_externalOnly:true}));return[...internal,...externalOnly].sort(sortRecords);
  }
  function appendComparisonRow(container,source,details,code,url){
    const row=document.createElement("div");row.className="comparisonRow";const sourceBlock=document.createElement("div");sourceBlock.className="comparisonSource";const heading=document.createElement(url?"a":"strong");heading.textContent=source;if(url){heading.href=url;heading.target="_blank";heading.rel="noopener noreferrer";}const meta=document.createElement("span");meta.textContent=details;sourceBlock.append(heading,meta);const value=document.createElement("div");value.className="comparisonCode";value.textContent=code||"—";row.append(sourceBlock,value);container.append(row);
  }
  function makeComparisonPanel(record){
    const panel=document.createElement("div");panel.className="cardPanel comparisonPanel";panel.hidden=true;const title=document.createElement("p");title.className="panelTitle";title.textContent="Порівняння джерел";panel.append(title);const matches=state.externalRecords.filter(external=>sameVehicle(record,external));const ownCode=record._externalOnly?"":record.lishi;const codes=matches.map(item=>normalize(item.lishi));const summary=document.createElement("div");
    if(record._externalOnly){summary.className="comparisonSummary pending";summary.textContent="Зовнішній запис відсутній у вашому каталозі LishiFinder.";}else if(record.alternativeLishi){summary.className="comparisonSummary conflict";summary.textContent="Основний код узято зі скріншота користувача; інший довідник збережено як примітку.";}else if(!matches.length){summary.className="comparisonSummary pending";summary.textContent="Зовнішні записи не імпортовані або відповідність не знайдена.";}else if(ownCode&&codes.every(code=>code===normalize(ownCode))){summary.className="comparisonSummary confirmed";summary.textContent="Збіг: усі імпортовані джерела вказують той самий Lishi.";}else{summary.className="comparisonSummary conflict";summary.textContent="Є розбіжність між вашим каталогом та зовнішніми джерелами.";}panel.append(summary);
    if(!record._externalOnly)appendComparisonRow(panel,record.source||"LishiFinder",[record.years,(record.markets||[]).map(value=>marketNames[value]||value).join(", "),record.checkedAt&&"Перевірено "+record.checkedAt].filter(Boolean).join(" • ")||"Вбудований запис",ownCode,safeSourceUrl(record.sourceUrl));
    if(!record._externalOnly&&record.alternativeLishi)appendComparisonRow(panel,record.alternativeSource||"Альтернативне джерело",record.alternativeNote||"Альтернативний код",record.alternativeLishi,safeSourceUrl(record.alternativeSourceUrl));
    matches.forEach(item=>appendComparisonRow(panel,item.source||"Імпорт",[item.years,item.note].filter(Boolean).join(" • ")||"Імпортований запис",item.lishi,safeSourceUrl(item.sourceUrl)));
    const links=document.createElement("div");links.className="comparisonLinks";sourceLinks.forEach(source=>{const link=document.createElement("a");link.href=source.url;link.target="_blank";link.rel="noopener noreferrer";link.textContent="Перевірити "+source.name+" ↗";links.append(link);});panel.append(links);return panel;
  }
  function makeNotePanel(record,button){
    const key=recordKey(record);const panel=document.createElement("div");panel.className="cardPanel noteEditor";panel.hidden=true;const label=document.createElement("label");const textarea=document.createElement("textarea");textarea.id="note-"+Math.random().toString(36).slice(2);label.htmlFor=textarea.id;label.textContent="Моя примітка для "+record.brand+" "+record.model;textarea.value=state.notes[key]||"";textarea.placeholder="Наприклад: перевірено на автомобілі клієнта, відрізняється ринок або потрібен інший інструмент…";textarea.maxLength=2000;const actions=document.createElement("div");actions.className="noteActions";const save=document.createElement("button");save.type="button";save.className="noteButton";save.textContent="Зберегти";const remove=document.createElement("button");remove.type="button";remove.className="noteButton delete";remove.textContent="Видалити";const status=document.createElement("span");status.className="noteStatus";
    save.addEventListener("click",()=>{const value=textarea.value.trim();if(value)state.notes[key]=value;else delete state.notes[key];if(storeJson(STORAGE_KEYS.notes,state.notes)){button.classList.toggle("hasNote",Boolean(value));button.textContent=value?"📝 Є примітка":"📝 Моя примітка";status.textContent=value?"Збережено на цьому пристрої":"Порожню примітку видалено";}else status.textContent="Не вдалося зберегти";});
    remove.addEventListener("click",()=>{textarea.value="";delete state.notes[key];storeJson(STORAGE_KEYS.notes,state.notes);button.classList.remove("hasNote");button.textContent="📝 Моя примітка";status.textContent="Видалено";});actions.append(save,remove,status);panel.append(label,textarea,actions);return panel;
  }
  function togglePanel(button,panel,otherButton,otherPanel){const opening=panel.hidden;if(otherPanel){otherPanel.hidden=true;otherButton.classList.remove("active");otherButton.setAttribute("aria-expanded","false");}panel.hidden=!opening;button.classList.toggle("active",opening);button.setAttribute("aria-expanded",String(opening));if(opening){const input=panel.querySelector("textarea");if(input)input.focus();}}
  function makeCard(record){
    ...
    const meta=document.createElement("div");meta.className="meta";const marketLabel=(record.markets||[]).map(value=>marketNames[value]||value).join(", ");meta.textContent=(record.years||"Рік не вказано")+" • "+(marketLabel||"Ринок не вказано")+" • "+(ignitionNames[record.ignition]||ignitionNames.unknown)+(record.chip?" • Чип: "+record.chip:"");
    ...
    if(record._externalOnly){const externalLabel=document.createElement("p");externalLabel.className="recordNote";externalLabel.textContent="Лише зовнішнє джерело: "+(record.source||"Імпорт");card.append(externalLabel);}else if(record.note){const note=document.createElement("p");note.className="recordNote";note.textContent=record.note;card.append(note);}
    const actions=document.createElement("div");actions.className="cardActions";const compareButton=document.createElement("button");compareButton.type="button";compareButton.className="cardAction";compareButton.textContent="📚 Порівняти джерела";compareButton.setAttribute("aria-expanded","false");const noteButton=document.createElement("button");noteButton.type="button";noteButton.className="cardAction"+(state.notes[recordKey(record)]?" hasNote":"");noteButton.textContent=state.notes[recordKey(record)]?"📝 Є примітка":"📝 Моя примітка";noteButton.setAttribute("aria-expanded","false");const comparisonPanel=makeComparisonPanel(record);const notePanel=makeNotePanel(record,noteButton);compareButton.addEventListener("click",()=>togglePanel(compareButton,comparisonPanel,noteButton,notePanel));noteButton.addEventListener("click",()=>togglePanel(noteButton,notePanel,compareButton,comparisonPanel));actions.append(compareButton,noteButton);card.append(actions,comparisonPanel,notePanel);if(state.mode==="compare"){comparisonPanel.hidden=false;compareButton.classList.add("active");compareButton.setAttribute("aria-expanded","true");}return card;
  }
  function renderMode(){
    const auto=state.mode==="auto",compare=state.mode==="compare";elements.query.placeholder=auto?"Toyota RAV4, Volkswagen Golf…":(compare?"Авто або код Lishi для порівняння…":"HU66, TOY43AT, HU127T…");
    elements.modeHelp.textContent=auto?"Введіть марку, модель, ринок або рік — каталог покаже відповідний код Lishi.":(compare?"Порівняйте ваш каталог з імпортованими дозволеними джерелами та власними примітками.":"Введіть код Lishi — каталог покаже автомобілі, для яких він зазначений.");
  ...
  function render(){
    ...
    const criteria=activeCriteria();const records=state.mode==="compare"?comparisonRecords():filteredRecords();elements.results.replaceChildren();elements.showAll.hidden=criteria||state.showAll;
    ...
    if(!records.length){const empty=document.createElement("div");empty.className="empty";empty.textContent=state.mode==="lishi"?"Код не знайдено серед перевірених записів.":(state.mode==="compare"?"Збігів немає. Імпортуйте дозволений CSV/JSON або змініть запит.":"Нічого не знайдено. Перевірте марку, ринок, рік або тип ключа.");elements.results.append(empty);return;}
